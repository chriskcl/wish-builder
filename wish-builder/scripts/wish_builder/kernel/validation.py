...
@dataclass(frozen=True, slots=True)
class _WritableSetSummary:
    """Canonical summaries for every path that one task may write."""

    owned_paths: _OwnershipSummary
    allowed_auxiliary_paths: _OwnershipSummary

    @property
    def fields(self) -> tuple[tuple[str, _OwnershipSummary], ...]:
        return (
            ("owned_paths", self.owned_paths),
            ("allowed_auxiliary_paths", self.allowed_auxiliary_paths),
        )

    @property
    def has_invalid(self) -> bool:
        return any(summary.has_invalid for _, summary in self.fields)
...
def _summarize_writable_set(task: Task | ManifestTask) -> _WritableSetSummary:
    """Compile owned and auxiliary grants as one complete writable set."""

    if type(task) not in {Task, ManifestTask}:
        raise TypeError("task must be an admitted Task model")
    return _WritableSetSummary(
        owned_paths=_summarize_ownership(task.owned_paths),
        allowed_auxiliary_paths=_summarize_ownership(
            task.allowed_auxiliary_paths
        ),
    )


def _writable_sets_overlap(
    left: _WritableSetSummary,
    right: _WritableSetSummary,
) -> tuple[str, str] | None:
    """Return the first conflicting field pair across two writable sets."""

    if (
        type(left) is not _WritableSetSummary
        or type(right) is not _WritableSetSummary
    ):
        raise TypeError("left and right must be writable-set summaries")
    for left_field, left_summary in left.fields:
        for right_field, right_summary in right.fields:
...
def _precompute_reachability(
    tasks: dict[str, Task | ManifestTask],
...
def _dependency_depth(tasks: dict[str, Task | ManifestTask]) -> int:
...
def validate_manifest(
    ...
    if not is_execution_manifest_model(manifest):
        raise TypeError(
            "validate_manifest accepts only an admitted execution manifest; "
    ...
    writable_summaries = {
        task.id: _summarize_writable_set(task)
    ...
    for task in manifest.tasks:
        ...
        for field_name, summary in writable_summaries[task.id].fields:
        ...
        if type(task) is Task and selected_phase in (
        ...
        if type(task) is Task and (
            task.status is TaskStatus.PR_OPEN
            or task.status in MERGED_OR_LATER_STATUSES
        ):
        ...
        if (
            type(task) is Task
            and task.status in MERGED_OR_LATER_STATUSES
            and task.squash_commit is None
        ):
            issues.append(
                _issue(
                    ValidationStage.LIFECYCLE,
        ...
        if (
            type(task) is Task
            and selected_phase is ValidationPhase.FINISH
            and task.status not in FINISHED_STATUSES
        ):
            issues.append(
                _issue(
                    ValidationStage.LIFECYCLE,
    ...
    # Suppress derivative barrier noise only for a task whose own dependency
    # subgraph is invalid. An unrelated unknown edge or cycle must not hide a
    # real barrier bypass elsewhere in the manifest.
    invalid_dependency_roots = {
        task.id
        for task in manifest.tasks
        if any(dependency_id not in tasks for dependency_id in task.depends_on)
    }
    invalid_dependency_roots.update(
        task_id for cycle in cycles for task_id in cycle
    )
    invalid_dependency_subgraphs = {
        task_id
        for task_id in tasks
        if task_id in invalid_dependency_roots
        or not reachability.get(task_id, frozenset()).isdisjoint(
            invalid_dependency_roots
        )
    }
    for wave in (1, 2):
        ...
        for task in current_wave:
            if task.id in invalid_dependency_subgraphs:
                continue
            missing = tuple(
                predecessor.id
    ...
    for index, left in enumerate(parallel):
        for right in parallel[index + 1 :]:
            if (
                ...
                continue
            left_summary = writable_summaries[left.id]
            right_summary = writable_summaries[right.id]
            if left_summary.has_invalid or right_summary.has_invalid:
                continue
            conflict = _writable_sets_overlap(left_summary, right_summary)
            if conflict is not None:
                left_field, right_field = conflict
                issues.append(
                    _issue(
                        ValidationStage.POLICY,
                        "manifest.parallel_ownership",
                        ("tasks", left.id, left_field),
                        "parallel_ownership_conflict",
                        (
                            f"Parallel tasks {left.id} and {right.id} have "
                            "overlapping complete writable sets."
                        ),
                        related_paths=(("tasks", right.id, right_field),),
                    )
                )
    ...
    if (
        ...
        and type(manifest.approvals.gate_b) is not GateApproval
    ):
        issues.append(
            _issue(
                ValidationStage.LIFECYCLE,
