...
from .serialization import MAX_CANONICAL_INTEGER, MIN_CANONICAL_INTEGER
...
def _string_list(
    ...
    *,
    ...
    for index, item in enumerate(value):
        decoded = _string(
            item,
            path + (_shape_segment(item, index),),
            issues,
            limit=limit,
    ...
    if requirements_raw is _MISSING:
        ...
        for index, item in enumerate(requirements_raw):
            decoded = _decode_requirement(
                item,
                ("requirements", _shape_segment(item, index)),
                issues,
            )
            if decoded is not None:
    ...
    if tasks_raw is _MISSING:
        ...
        for index, item in enumerate(tasks_raw):
            decoded = _decode_task(
                item,
                ("tasks", _shape_segment(item, index)),
                issues,
            )
            if decoded is not None:
...
def decode_manifest_bytes(
    ...
    *,
    ...
    """Decode untrusted UTF-8 JSON bytes through the sole raw manifest boundary."""
